...
# remove last week
dm2 = subset(dm2, is.na(dm2$weekly.deaths.lag) == F)

# formatting for glmmpql
dm2$FIPS = as.factor(dm2$FIPS)

# set minimum cases to 1 for defined offset
ind = which(dm2$weekly.cases == 0)
dm2$weekly.cases[ind] = 1

ind = which(dm2$weekly.deaths.lag > dm2$weekly.cases)
dm2$weekly.cases[ind] = dm2$weekly.deaths.lag[ind]
...
# model
model = glmmPQL( 
  fixed = weekly.deaths.lag ~ offset(log(weekly.cases)) + State +
...
# saving key results
# write.csv(dt, 
#          file="~/Desktop/Covid disparities/health disparity/Output/CFR Weekly Multivariable.csv",
#          row.names=F)
